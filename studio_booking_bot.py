"""
🎵 MACHATA STUDIO — Бот бронирования
Академия звука для профессионалов и творческих людей

Полнофункциональная версия с маркетинговым подходом + VIP-скидки:
- премиум текстовое наполнение
- интуитивный UX
- поэтапность (шаг 1/4, шаг 2/4, шаг 3/4, шаг 4/4)
- убеждающие элементы
- система VIP-пользователей с персональными скидками
- две опции записи: самостоятельная работа и со звукорежем
- контакты через Telegram и звонки (WhatsApp удалён)
- краткие тексты без тавтологии
...
# ===== VIP ПОЛЬЗОВАТЕЛИ (добавляй сюда ID пользователей, которым нужна скидка) =====
# Как получить ID: запусти бота, напиши /start, в консоли увидишь chat_id
# Формат: {chat_id: {'name': 'Имя', 'discount': 20}}  <- 20% скидка
VIP_USERS = {
    123456789: {'name': 'Иван Рок', 'discount': 20},      # пример: 20% скидка
    987654321: {'name': 'Мария Вокал', 'discount': 15},   # пример: 15% скидка
    555444333: {'name': 'Миша Продакшн', 'discount': 25}, # пример: 25% скидка
}

DEFAULT_CONFIG = {
    'prices': {
        'repet': 700,           # 🎸 Репетиция 700 ₽/час
        'studio': 800,          # 🎧 Аренда студии (самост. работа) 800 ₽/час
        'full': 1500,           # ✨ Аренда со звукорежем 1500 ₽
    },
...
def get_user_discount(chat_id):
    """Получить скидку пользователя (если VIP)"""
...
def service_inline_keyboard(service_type):
    """Выбор услуги для конкретного типа"""
...
def times_keyboard(chat_id, date_str, service):
    """Выбор времени с пересчётом цены"""
    ...
    if selected:
        ...
        # Если не VIP, применяем обычные скидки за количество часов
        elif len(selected) >= 5:
            discount_percent = 15
...
def get_welcome_text(chat_id):
    """Приветственный текст с учётом VIP статуса"""
    vip_badge = ""
...
def send_welcome(m):
    ...
    print(f"👤 Пользователь: {m.from_user.first_name} | ID: {chat_id}")  # Для копирования ID
...
Профессиональное оборудование, полный контроль звука.
Работай самостоятельно или с нашим звукорежиссером  
...
🚇 Метро Тульская: [12 минут пешком]
...
Телефон: {STUDIO_CONTACT_2}
Telegram: {STUDIO_TELEGRAM_2}
...
def process_text_steps(m):
    ...
    if step == 'name':
        ...
        # Проверяем количество цифр (должно быть ровно 11 для российского номера)
        ...
        # Добавляем кнопку "Пропустить" на этапе комментария
        kb = types.ReplyKeyboardMarkup(row_width=1, resize_keyboard=True)
...
def complete_booking(chat_id):
    ...
    # Если не VIP, применяем обычные скидки
    elif duration >= 5:
