...
def format_welcome(chat_id):
    """Профессиональное форматированное приветствие с маркетинговым дизайном"""
    vip_badge = ""
...
"""
    ...
    return f"""<pre>╔═══════════════════════════════════╗</pre>
<pre>║     🎵 MACHATA STUDIO     ║</pre>
<pre>╚═══════════════════════════════════╝</pre>

<b>🔥 Где рождается настоящая музыка</b>

<blockquote>Ты попал в место, где звук становится искусством.
Профессиональная студия мирового уровня в самом сердце Москвы.</blockquote>

<pre>━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━</pre>

<b>🎯 ЧТО МЫ ПРЕДЛАГАЕМ:</b>

<code>🎸 РЕПЕТИЦИЯ</code> — <b>700 ₽/час</b> ⚡
   🎤 Идеальная акустика для твоей музыки
   🎹 Все инструменты готовы к игре
   ☕ Кофе, чай, уют — всё включено
   💫 Атмосфера, где рождаются хиты

<code>🎧 СТУДИЯ</code> — <b>800 ₽/час</b> 💎
   🎚️ Премиум-оборудование мирового класса
   🔇 Звукоизоляция класса А
   🎛️ Полный контроль над каждым звуком
   🎬 Твой трек будет звучать как в топ-чартах

<code>✨ СТУДИЯ + ЗВУКОРЕЖИССЁР</code> — <b>1500 ₽/час</b> 👑
...
<pre>╔═══════════════════════════════════╗</pre>
<pre>║  ⚡ Забронируй за 30 секунд!  ║</pre>
<pre>╚═══════════════════════════════════╝</pre>

🎵 <b>Твоя музыка ждёт тебя!</b>{vip_badge}"""

def format_prices(chat_id):
    """Профессиональные форматированные тарифы с маркетинговым дизайном"""
    vip_info = ""
    if is_vip_user(chat_id):
        vip_discount = VIP_USERS[chat_id]['discount']
        vip_info = f"""
...
<code>🎧 СТУДИЯ (САМОСТОЯТЕЛЬНО)</code>
   <b>800 ₽/час</b> 💎
...
<code>✨ СТУДИЯ СО ЗВУКОРЕЖЕМ</code>
   <b>1500 ₽/час</b> 👑
...
<pre>╔═══════════════════════════════════╗</pre>
<pre>║  ⚡ Забронируй прямо сейчас!  ║</pre>
<pre>╚═══════════════════════════════════╝</pre>

🎵 <b>Твоя музыка ждёт тебя!</b>"""

def format_location():
    """Профессиональная форматированная информация о локации"""
    return """<pre>╔═══════════════════════════════════╗</pre>
<pre>║     📍 КОНТАКТЫ     ║</pre>
<pre>╚═══════════════════════════════════╝</pre>
...
💡 <i>Нажми на кнопки ниже, чтобы открыть карту</i>"""
...
def format_admin_booking(booking):
    ...
    status = booking.get('status', 'pending')
    status_map = {
        'pending': '🟡 <b>Ожидает оплаты</b>',
        'awaiting_payment': '🟡 <b>Ожидает оплаты</b>',
        'paid': '🟢 <b>Оплачено</b> ✅',
        'cancelled': '🔴 <b>Отменено</b> ❌',
    }
    status_text = status_map.get(status, f'⚪ <b>{status}</b>')
    
    return f"""<pre>╔═══════════════════════════════════╗</pre>
<pre>║     📋 БРОНЬ #{booking.get('id', 'N/A')}     ║</pre>
<pre>╚═══════════════════════════════════╝</pre>
